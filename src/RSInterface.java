...
public class RSInterface {
	...
	public static void lootingBag(TextDrawingArea[] tda) {
		...
		addHoverButton(26702, 454,  16, 16, "Close", 0, 26703, 1);
		addHoveredButton(26703, 455,  16, 16, 26704);
		addText(26705, "Looting bag", tda, 2, 0xFF9900, true, true);
		itemContainer(26706, 4, 1, 4, 7, false,"Withdraw-1","Withdraw-5","Withdraw-All","Withdraw-X");
	...
	public static void quickPrayers(final TextDrawingArea[] TDA) {
		...
		int j = 17202;
		for (int k = 630; j <= 17229 || k <= 655; ++k) {
			addConfigButton(j, 17200, 2, 1, 14, 15,
					"Select", 0, 1, k);
			j++;
		}
		...
		setBounds(25053, 41, 212, i++, localRSInterface);
		setBounds(25055, 79, 212, i++, localRSInterface);
		...
		j++;
		addPrayerWithTooltip(25002, 0, 84, 3, 0, 25054, "Activate @lre@Burst of Strength");
		...
		j++;
		addPrayerWithTooltip(25052, 0, 609, 72, j, 25104, "Activate @lre@Rigour");
		j++;
		addPrayerWithTooltip(25054, 0, 610, 76, j, 25106, "Activate @lre@Augury");
	}

	private static void addPrayerWithTooltip(int i, int j, int k, int l, int j2, int m, String string) {
